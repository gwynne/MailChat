...
@protocol ELWAsyncPOP3SocketDelegate;

enum { kPOP3DefaultPort = 110, kPOP3SDefaultPort = 995 };

// RFCs 1939, 2449, 2595, 5034
@interface ELWAsyncPOP3Socket : GCDAsyncSocket
...
//	"delegate" property.
@property(atomic,weak) id<ELWAsyncPOP3SocketDelegate> pop3Delegate;
...
@protocol ELWAsyncPOP3SocketDelegate <NSObject, GCDAsyncSocketDelegate>

@optional

// The delegate will receive a disconnect message immediately after this call if
//	the server closes the connection.
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didReceivePOP3Error:(NSString *)serverMessage;
...
//	implemented.
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didSayHello:(NSString *)helloMessage;
...
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didReceiveCapabilityList:(NSArray *)capabilities;
...
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didAuthenticateWithMethod:(NSString *)authMethod;
...
- (NSData *)pop3Socket:(ELWAsyncPOP3Socket *)socket didReplyToAuthenticationRequest:(NSData *)serverReply;

// The POP3 server replied to a STAT request with the given number of messages
//	and message byte count.
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didReceiveMaildropCount:(NSInteger)numMessages maildropSize:(NSInteger)octets;
...
//	NSString.
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didReceiveMessageInfoList:(NSDictionary *)messageSizes;
...
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didReceiveMessage:(NSData *)message withNumber:(NSInteger)messageNum;

// The POP3 server replied with success to a DELE command (wasDeleted == true)
//	or a RETR command (wasDeleted == false, messageNum == -1).
- (void)pop3Socket:(ELWAsyncPOP3Socket *)socket didMarkMessage:(NSInteger)messageNum deleted:(bool)wasDeleted;
